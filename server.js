...
const malkhanaSchema = new mongoose.Schema({
  ...
  // ── 3. Case / FIR ─────────────────────────────────────────────────────────
  case_fir_no: { type: String, default: '' },
  case_date:   { type: Date },

  // ── 4. Property type ──────────────────────────────────────────────────────
  property_type: { type: String, default: '' },

  // ── 5. Description & identification mark ──────────────────────────────────
  property_description: { type: String, default: '' },
  identification_mark:  { type: String, default: '' },

  // ── 6. Value ──────────────────────────────────────────────────────────────
  property_value: { type: String, default: '' },

  // ── 7. Where / when / by whom / circumstances ─────────────────────────────
  found_place:         { type: String, default: '' },
  found_date:          { type: Date },
  found_by_officer:    { type: String, default: '' },
  found_circumstances: { type: String, default: '' },

  // ── 8. Date received at police station ────────────────────────────────────
  received_at_police_station_date: { type: Date },
  ...
  disposal_method: { type: String, default: '' },

  // ── 11. Receipt file / sale details ───────────────────────────────────────
  receipt_file_entry_no: { type: String, default: '' },
  sale_amount:           { type: String, default: '' },
  buyer_name:            { type: String, default: '' },
  buyer_address:         { type: String, default: '' },
  receipt_check_no:      { type: String, default: '' },
  receipt_check_date:    { type: Date },

  // ── 12. Custody receipt & delay reason ────────────────────────────────────
  custody_receipt_no: { type: String, default: '' },
...
// Empty string → null for Date fields (prevents Mongoose cast errors from FormData)
const DATE_FIELDS = [
  'case_date', 'found_date', 'received_at_police_station_date',
  'disposal_date', 'receipt_check_date'
];
function sanitizeDates(body) {
  DATE_FIELDS.forEach(f => { if (body[f] === '' || body[f] === undefined) body[f] = null; });
  return body;
}
...
app.post('/api/malkhana', authenticateToken, upload.array('photos', 5), async (req, res) => {
  try {
    ...
    if (body.entry_no) body.entry_no = parseInt(body.entry_no);
    if (req.files?.length) {
...
app.put('/api/malkhana/:entry_no', authenticateToken, async (req, res) => {
  try {
    ...
    const updated = await MalkhanaRegister.findOneAndUpdate(
      { entry_no: req.params.entry_no }, body, { new: true, runValidators: true }
    );
...
app.post('/api/malkhana/:entry_no/photos', authenticateToken, upload.array('photos', 5), async (req, res) => {
  try {
    ...
    await record.save();
    res.json({
      success: true, message: `${toAdd.length} photo(s) added`, total_photos: record.photos.length,
...
app.get('/api/malkhana/:entry_no/photos', authenticateToken, async (req, res) => {
  try {
    ...
    if (!record) return res.status(404).json({ success: false, message: 'Record not found' });
    res.json({
      success: true, count: record.photos.length,
      photos: record.photos.map(({ _id, filename, mimetype, size, uploaded_at }) => ({ _id, filename, mimetype, size, uploaded_at }))
    });
  } catch (e) { res.status(500).json({ success: false, message: e.message }); }
});
...
app.get('/', async (req, res) => {
  try {
    ...
    res.json({
      ...
      schema_columns: {
        "1":  "entry_no",
        "2":  "owner_name, owner_address",
        "3":  "case_fir_no, case_date",
        "4":  "property_type",
        "5":  "property_description, identification_mark",
        "6":  "property_value",
        "7":  "found_place, found_date, found_by_officer, found_circumstances",
        "8":  "received_at_police_station_date",
        "9":  "storage_location",
        "10": "disposal_date, disposal_method",
        "11": "receipt_file_entry_no, sale_amount, buyer_name, buyer_address, receipt_check_no, receipt_check_date",
        "12": "custody_receipt_no, delay_reason"
      }
    });
